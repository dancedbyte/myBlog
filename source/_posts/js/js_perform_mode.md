...
1. 所有同步任务**作为宏任务**先在主线程上执行，形成一个执行栈（execution context stack）。

2. 主线程之外，还存在一个"任务队列"（task queue）。异步任务又分为宏任务和微任务，宏任务是一个一个按顺序执行的，每执行完一个宏任务就要去检查微任务队列。只要异步任务有了运行结果，就在"任务队列"之中放置一个事件。

3. 一旦"执行栈"中的所有同步任务执行完毕，系统就会读取"任务队列"，看看里面有哪些事件。则那些对应的异步任务就结束等待状态开始执行。

4. 主线程不断重复上面的第三步即是事件循环。
